...
from document_search import load_documents_into_search_engine, search_engine
...
async def health_check(request: Request):
    ...
    try:
        ...
        if not is_heartbeat:
            logger.info(
                f"Health check requested by session {session_id[:12]}... (type: {health_check_type})"
            )
            ...
            if health_check_type == "full" and ai_status == "ready":
                try:
                    ...
                    test_future = asyncio.wait_for(
                        asyncio.create_task(test_ai_connection()), timeout=3.0
                    )
...
async def keep_alive():
    ...
    return {
        ...
        "timestamp": datetime.datetime.now().isoformat(),
        "uptime_seconds": (
            (datetime.datetime.now() - app_start_time).total_seconds()
    ...
    try:
        ...
        if ai_result.get("sources"):
            source_info = "\n\n📚 Sources consulted: " + ", ".join(
                [f"{src['filename']} ({src['similarity']:.2f})" for src in ai_result["sources"]]
            )
        ...
        if ai_result.get("model_used"):
            logger.info(
                f"AI response generated using {ai_result['model_used']} with {ai_result.get('chunks_used', 0)} document chunks"
            )

        response = ChatResponse(
            response=response_text, timestamp=datetime.datetime.now().isoformat()
        )

        logger.info(
            f"Sending {ai_result['type']} response with {len(relevant_chunks)} relevant chunks"
        )
        return response
...
async def chat_stream_endpoint(chat_message: ChatMessage):
    ...
    try:
        ...
        async def generate_stream():
            try:
                async for chunk_data in qsr_assistant.generate_response_stream(
    ...
    try:
        ...
        if range_header:
            ...
            try:
                ...
                logger.info(
                    f"Serving partial content: {filename} ({byte_start}-{byte_end}/{file_size})"
                )
        ...
        return StreamingResponse(generate_file(), headers=headers, media_type=content_type)
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            search_engine.add_document(doc_id=doc_id, text=extracted_text, filename=file.filename)
...
async def list_documents():
    ...
    try:
        ...
        response = DocumentListResponse(documents=documents, total_count=len(documents))
...
async def get_ai_status():
    ...
    try:
        ...
        return AIStatusResponse(
            ai_available=ai_available, model_name=model_name, status_message=status_message
        )
    except Exception as e:
